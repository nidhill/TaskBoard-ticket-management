    ...
    <MainLayout>
      ...
      <div className="mb-9">
        <h1
          ...
          className="text-[52px] font-bold leading-[1.05] tracking-tight text-foreground mb-2"
        >
          Welcome back
        </h1>
        <p className="text-[15px] text-muted-foreground">Your workspace at a glance.</p>
      </div>

      {/* ── Quick Actions ── */}
      {quickActions.length > 0 && (
        <div className="mb-8">
          <p className="text-[10px] font-bold uppercase tracking-[1.8px] text-muted-foreground mb-4">
            Quick Actions
          </p>
          <div className="flex flex-wrap gap-3">
            {quickActions.map((action, i) => (
              <Link
                key={i}
                to={action.href}
                className="group flex flex-col items-center justify-center gap-3 bg-card border border-border rounded-xl px-6 py-5 min-w-[130px] flex-1 hover:border-border hover:shadow-md transition-all select-none"
                style={{ textDecoration: 'none', maxWidth: 200 }}
              >
                <div className="w-10 h-10 rounded-xl bg-muted/60 group-hover:bg-muted flex items-center justify-center transition-colors relative">
                  <action.icon className="w-4.5 h-4.5 text-muted-foreground" style={{ width: 18, height: 18 }} />
                ...
                <span className="text-[12px] font-semibold text-muted-foreground group-hover:text-foreground text-center leading-tight transition-colors">
                  {action.label}
                </span>
              </Link>
            ))}
