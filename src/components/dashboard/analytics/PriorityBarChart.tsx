        ...
        <div className="bg-card rounded-xl border border-border shadow-sm h-full">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border/50">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Current workload distribution</p>
                </div>
                <button className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors">
            ...
            {total === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                    ...
                    <p className="text-xs text-muted-foreground/70 max-w-[200px]">Insufficient data to generate breakdown</p>
                </div>
            ) : (
                    ...
                    <div className="flex gap-5 mt-3">
                        {Object.keys(priorityLabels).map((priority) => (
                            <div key={priority} className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-semibold text-foreground">{(stats as any)?.[priority] || 0}</span>
                            </div>
                        ))}
                    </div>
                </div>
