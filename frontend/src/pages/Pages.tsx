...
export default function Pages() {
  ...
  const handleCreatePage = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Your page request has been submitted successfully',
      });
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to create page request',
        variant: 'destructive',
      });
    } finally {
  ...
  const handleStatusUpdate = async () => {
    if (!selectedPage || !newStatus) return;

    setIsUpdating(true);
    try {
      await api.put(`/pages/${selectedPage._id}`, { status: newStatus });
  ...
  const handleApprove = async (page: Page) => {
    try {
      await api.put(`/pages/${page._id}`, { status: 'approved' });
      toast({
        title: 'Page Approved',
        description: 'The page has been approved successfully',
      });
      fetchPages();
    } catch (error: any) {
      toast({
  ...
  const handleReject = async (page: Page) => {
    try {
      await api.put(`/pages/${page._id}`, { status: 'draft' }); // Rejected pages go back to draft
      toast({
        title: 'Page Rejected',
        description: 'The page has been rejected',
      });
      fetchPages();
    } catch (error: any) {
      toast({
  ...
  const openStatusDialog = (page: Page) => {
  ...
  const filteredPages = pages.filter((page) => {
    const projectName = page.projectId?.name || '';
    const matchesSearch = projectName.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = statusFilter === 'all' || page.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const pendingApprovals = pages.filter(p => p.status === 'approval_pending').length;
  ...
  return (
    <MainLayout>
      <PageHeader
        title="Pages"
        description="View and manage all pages across your projects."
        actions={
          permissions.canCreatePage && (
            <Dialog open={isRequestDialogOpen} onOpenChange={setIsRequestDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Request New Page</DialogTitle>
                  <DialogDescription>
                    Submit a request for a new page to be developed for a project.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder="e.g. Landing Page, Dashboard, etc."
                      value={newPageName}
                  ...
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground">
                      📝 Your page request will be submitted for approval by the Department Head.
      ...
      {/* Role Info Banner */}
      {
        role && (
          <div className={cn(
            ...
            <p className="text-sm text-muted-foreground">
              {role === 'requester' && '📄 As a Requester, you can request new pages and view their status.'}
              {role === 'developer' && '🔧 As a Developer, you can update page status through the workflow.'}
              {role === 'department_head' && (
                <>{pendingApprovals > 0 ? `✅ As a Department Head, you can approve/reject pages. ${pendingApprovals} pages pending approval.` : '✅ As a Department Head, you can approve/reject pages.'}</>
              )}
              {role === 'tech_admin' && '🔑 As a Tech Admin, you have full control over all pages.'}
            </p>
          </div>
        )
      }
      ...
      <div className="space-y-4">
        {filteredPages.map((page) => (
          <Card key={page._id} className="glass-card hover-lift">
            <CardContent className="p-4">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-foreground">{page.pageName || `Page #${page._id.slice(-6)}`}</h3>
                    <StatusPill status={page.status as any} />
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{page.projectId?.name || 'Unknown Project'}</span>
                    </div>
                    {page.assignedTo ? (
                      <div className="flex items-center gap-2">
                        <Avatar className="w-5 h-5">
                          <AvatarImage src={page.assignedTo.avatar_url} />
                          <AvatarFallback className="text-[10px]">
                            {page.assignedTo.name.charAt(0)}
                          </AvatarFallback>
                        </Avatar>
                        <span>{page.assignedTo.name}</span>
                      </div>
                    ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {permissions.canApprovePage && page.status === 'approval_pending' && (
                      <>
                        <Button
                          size="sm"
                          variant="outline"
                    ...
                    {permissions.canUpdatePageStatus && page.status !== 'delivered' && page.status !== 'approval_pending' && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="h-8"
                        onClick={() => openStatusDialog(page)}
                      >
                    ...
                    {permissions.canUnlockPage && page.ticketUsed >= page.maxTickets && (
                    ...
                    {(permissions.canDeletePage || user?.role === 'admin' || (!page.createdBy || (typeof page.createdBy === 'object' ? (page.createdBy as any)._id : page.createdBy) === user?.id)) && (
                      <AlertDialog>
                        ...
                        <AlertDialogContent onClick={(e) => e.stopPropagation()}>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              "{page.pageName}" and all associated tickets.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
      ...
      <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Update Page Status</DialogTitle>
            <DialogDescription>
              Change the status of this page to move it through the workflow.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select value={newStatus} onValueChange={setNewStatus}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="draft">Draft (Rejected)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
